...
function buildIndexPage(tree, graph, topic, allSlugs, wikiDir) {
  const EMOJIS = ['🎯', '📊', '⚡', '🔬', '🛡️', '📐', '🧠', '💡', '🔧', '📈', '🌐', '🏗️'];
  ...
  const domainCards = domains.map((domain, i) => {
    ...
    return `<a class="entry-card" href="${domain.children[0]?.page_slug || domain.page_slug}.html">
...
<body>
  ...
  <script>
  (function(){
    var md = ${JSON.stringify(buildMarkmapMd(tree))};
...
function buildMarkmapMd(tree) {
  const lines = [`# ${tree.label || tree.title || tree.page_slug}`];
  for (const child of (tree.children || [])) {
    lines.push(`## [${child.label || child.title}](${child.page_slug}.html)`);
...
function run(wikiDir, treePath, outputDir, topic) {
  ...
  // Index page (demo-parity style)
  fs.writeFileSync(path.join(siteDir, 'index.html'), buildIndexPage(tree, graph, topic, allSlugs, wikiDir), 'utf-8');
