...
const { chatCompletion, getModel } = require('../lib/llm-client');
const { recordDecision } = require('../lib/wiki-engine/tech-decisions');
...
async function crystallize(sourceId, contentPath, outputDir, topic, options = {}) {
  ...
  fs.writeFileSync(outputPath, JSON.stringify(allAtoms, null, 2), 'utf-8');

  recordDecision(outputDir, topic, {
    stage: 'stage-1',
    name: 'crystallize LLM',
    chosen: getModel(),
    rationale: `使用 function calling (emit_atoms tool)。tool schema 把 claim/evidence_quote/evidence_char_offset/kind/draft_tags 全部声明为 required，杜绝字段缺失。`,
    known_limits: [
  ...
  console.log(`[Stage 1] Crystallized ${allAtoms.length} atoms from ${sourceId}`);
  return allAtoms;
}
