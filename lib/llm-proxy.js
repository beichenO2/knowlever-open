 ...
 *
 * Calls PolarPrivate's /v1/ unified gateway (same protocol as privportal-sdk).
 * Port discovery: POLARPRIVATE_URL env → POLARPRIVATE_PORT env → LLM_BASE_URL → default 12790.
 *
...
function getPolarPrivateUrl() {
  if (process.env.POLARPRIVATE_URL) return process.env.POLARPRIVATE_URL.replace(/\/$/, '');
  const port = process.env.POLARPRIVATE_PORT || process.env.LLM_PORT || '12790';
  return process.env.LLM_BASE_URL || `http://127.0.0.1:${port}`;
}
...
/**
 * Embedding via PolarPrivate /v1/embeddings.
 */
