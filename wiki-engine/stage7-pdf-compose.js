...
${htmlContent}
  ...
  // Generate actual PDF using weasyprint (if available)
  const { spawnSync } = require('child_process');
  const pdfPath = path.join(pdfDir, `${topic}-handbook.pdf`);
  const wpResult = spawnSync('weasyprint', [htmlPath, pdfPath], {
    stdio: 'pipe', timeout: 120_000,
  });
  if (wpResult.status === 0) {
    console.log(`[Stage 7] ✅ PDF generated: ${pdfPath}`);
  } else {
    // Fallback: try pandoc
    const pandocResult = spawnSync('pandoc', [mdPath, '-o', pdfPath, '--pdf-engine=weasyprint'], {
      stdio: 'pipe', timeout: 120_000,
    });
    if (pandocResult.status === 0) {
      console.log(`[Stage 7] ✅ PDF generated (pandoc): ${pdfPath}`);
    } else {
      console.log(`[Stage 7] ⚠️ PDF auto-generation not available (install weasyprint: pip install weasyprint)`);
      console.log(`[Stage 7]    Manual: open ${htmlPath} in browser → Print → Save as PDF`);
    }
  }

  // Generate .docx using officecli (if available)
  const docxPath = path.join(pdfDir, `${topic}-handbook.docx`);
  const cliCheck = spawnSync('which', ['officecli'], { stdio: 'pipe' });
  if (cliCheck.status === 0) {
    const createResult = spawnSync('officecli', ['create', docxPath], { stdio: 'pipe' });
    if (createResult.status === 0) {
      // Add content using pandoc md→docx
      const pandocDocx = spawnSync('pandoc', [mdPath, '-o', docxPath, '--reference-doc=' + docxPath], {
        stdio: 'pipe', timeout: 60_000,
      });
      if (pandocDocx.status !== 0) {
        // Fallback: direct pandoc without reference
        spawnSync('pandoc', [mdPath, '-o', docxPath], { stdio: 'pipe', timeout: 60_000 });
      }
      console.log(`[Stage 7] ✅ DOCX generated: ${docxPath}`);
    }
  }
