...
⚠️ 禁止：
- 禁止编虚构故事/寓言/角色（不要出现"小林""工程师发现""在遥远的…"）
- 禁止用类比代替因果解释（"像交响乐""像水流"这种修辞不要）
- 禁止泛泛而谈（每一句都必须有具体的技术内容）

✅ 要求：
- 用工程师写技术报告的语气
- 公式必须完整（变量定义 + 推导 + 代入具体数值算一遍）
- 每个知识原子的核心信息必须体现在正文中
- 如果 atom 是简单事实/定义，直接用"**名词** — 一句话解释"格式`;

  const nonLeafStrategy = `【非叶子节点写法 — 问题驱动总结】
...
3. **公式和化学式**：数学用 $...$（inline）或 $$...$$（display），化学式也用 LaTeX。完整给出公式和变量定义。
